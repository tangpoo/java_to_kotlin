...
import travelator.uncapsulatedcollection.TravelMethod.*
...
class RouteTests {
    ...
    fun replaceJourney() {
        ...
        val journey3 =
            Journey(alresford, winchester, someTime(), someTime(), BUS)
        val route =
            listOf(
                journey1,
                journey2,
        ...
        val replacement =
            Journey(alton, alresford, someTime(), someTime(), RAIL)

        assertEquals(
            listOf(journey1, replacement, journey3),
            route.withItemAt(1, replacement).journeys
        )
    }
}
